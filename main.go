...
import (
	...
	"github.com/SanjaySinghRajpoot/interactWrapper/routes"
	"github.com/SanjaySinghRajpoot/interactWrapper/runner"
...
func main() {
	...
	for _, interactshURL := range interactshURLs {
		...
		// set the working URL
		controller.GetURLTemp = interactshURL
	}
		...
		if !cliOptions.JSON {
			...
			case "dns":
				if noFilter || cliOptions.DNSOnly {
					str := fmt.Sprintf("[%s] Received DNS interaction (%s) from %s at %s", interaction.FullId, interaction.QType, interaction.RemoteAddress, interaction.Timestamp.Format("2006-01-02 15:04:05"))
					builder.WriteString(str)
					if cliOptions.Verbose {
						builder.WriteString(fmt.Sprintf("\n-----------\nDNS Request\n-----------\n\n%s\n\n------------\nDNS Response\n------------\n\n%s\n\n", interaction.RawRequest, interaction.RawResponse))
					}

					utils.AppendValue(controller.GetData, interaction.FullId, str)
					writeOutput(outputFile, builder)
				}
			case "http":
				if noFilter || cliOptions.HTTPOnly {
					str := fmt.Sprintf("[%s] Received HTTP interaction from %s at %s", interaction.FullId, interaction.RemoteAddress, interaction.Timestamp.Format("2006-01-02 15:04:05"))
					builder.WriteString(str)
					if cliOptions.Verbose {
						builder.WriteString(fmt.Sprintf("\n------------\nHTTP Request\n------------\n\n%s\n\n-------------\nHTTP Response\n-------------\n\n%s\n\n", interaction.RawRequest, interaction.RawResponse))
					}
			...
			case "smtp":
				if noFilter || cliOptions.SmtpOnly {
					str := fmt.Sprintf("[%s] Received SMTP interaction from %s at %s", interaction.FullId, interaction.RemoteAddress, interaction.Timestamp.Format("2006-01-02 15:04:05"))
					builder.WriteString(str)
					if cliOptions.Verbose {
			...
			case "ftp":
				if noFilter {
					str := fmt.Sprintf("Received FTP interaction from %s at %s", interaction.RemoteAddress, interaction.Timestamp.Format("2006-01-02 15:04:05"))
					builder.WriteString(str)
					if cliOptions.Verbose {
						builder.WriteString(fmt.Sprintf("\n------------\nFTP Interaction\n------------\n\n%s\n\n", interaction.RawRequest))
					}
			...
			case "responder", "smb":
				if noFilter {
					str := fmt.Sprintf("Received Responder/Smb interaction at %s", interaction.Timestamp.Format("2006-01-02 15:04:05"))
					builder.WriteString(str)
					if cliOptions.Verbose {
			...
			case "ldap":
				if noFilter {
					str := fmt.Sprintf("[%s] Received LDAP interaction from %s at %s", interaction.FullId, interaction.RemoteAddress, interaction.Timestamp.Format("2006-01-02 15:04:05"))
					builder.WriteString(str)
					if cliOptions.Verbose {
						builder.WriteString(fmt.Sprintf("\n------------\nLDAP Interaction\n------------\n\n%s\n\n", interaction.RawRequest))
					}
